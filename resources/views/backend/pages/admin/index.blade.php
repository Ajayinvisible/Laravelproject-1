                        ...
                        @if($adminData->count()>0)
                          @foreach ($adminData as $key=>$admin) 
                            <tr>
                              ...
                              <td>
                                @if($admin->gallery)
                                  <img src="{{url($admin->gallery->image)}}" style="width: 40px; height:40px; border-radious:50%;" alt="">
                                @endif
                              </td>
                              <td>
                                <form action="{{route('admin.destroy',$admin->id)}}" method="POST">
                                  @csrf
                                  @method('DELETE')
                                  <a href="" class="btn btn-secondary" title="Show"><i class="fa-solid fa-eye"></i></a>
